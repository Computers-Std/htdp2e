#lang htdp/isl+
; Exercise 361. Design eval-all. Like eval-function* from exercise
; 359, this function consumes the representation of an expression and
; a definitions area. It produces the same value that DrRacket shows
; if the expression is entered at the prompt in the interactions area
; and the definitions area contains the appropriate definitions. Hint
; Your eval-all function should process variables in the given
; expression like eval-var-lookup in exercise 355.
...
(check-expect (eval-all (make-fun 'f 4) da1) 7)
(check-expect (eval-all (make-fun 'h 4) da1) 18)
(check-expect (eval-all (make-fun 'area-of-circle 1) da2) (/ 157 50))
(check-expect (eval-all (make-fun 'volume-of-10-cylinder 1) da2) (/ 157 5))
(check-expect (eval-all (make-mul 3 'close-to-pi) da2) (/ 471 50))
